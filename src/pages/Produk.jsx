...
function Produk() {
  ...
  const getLiquids = (kategori) => {
    ...
    axios
      .get(
        `https://651eb45144a3a8aa4768d77a.mockapi.io/liquid?kategori=${
          kategori ? kategori : ``
        }`
      )
      .then((response) => {
  ...
  return (
    <section>
      ...
      <div className="block md:flex items-center justify-center mt-5">
        <div>
          <div className="block md:flex rounded-md px-5 md:px-0">
            <input
              ...
              type="text"
              className="py-2 px-3 mx-auto md:pr-11 text-center md:text-left rounded-md md:rounded-r-none w-full md:w-100 border-gray-200 outline outline-1 shadow-sm rounded-l-md text-sm focus:z-10 focus:outline-2 bg-white border-gray-700 text-[#1E2022]"
            />
            <button
              onClick={handleSearch}
              className="px-4 h-9 mb-4 md:mb-0 items-center mt-3 md:mt-0 w-full md:w-0 min-w-fit rounded-md md:rounded-l-none rounded-r-md border border-l-0 border-gray-200 text-sm text-gray-500 bg-[#1E2022] border-gray-700 text-white hover:bg-[#484d4e]"
            >
              Search
            </button>
          </div>
        ...
        <div
          className="flex justify-center md:block md:ml-10"
          ref={dropdownRef}
        >
          <button
            onClick={toggleDropdown}
            className="cursor-pointer text-[#1E2022] bg-white rounded-md p-1 px-4 outline outline-2 outline-[#1E2022] flex items-center"
          >
            Kategori&nbsp;&nbsp;
            {showDropdown == false ? (
          ...
          {showDropdown && (
            <ul className="absolute z-50 mt-10 md:mt-2 rounded-md bg-white text-gray-800 hover:outline-none text-center">
              <li>
                <a
                  onClick={() => getLiquids("")}
                  className="cursor-pointer block text-sm px-5  py-2 hover:bg-[#1E2022] rounded-sm hover:outline-none hover:text-white"
                >
                  All category
                </a>
              </li>
      ...
      {loading ? (
        <div className="h-72 flex items-center justify-center font-medium text-2xl">
          Loading....
        </div>
      ) : (
              ...
              <section className="mx-auto my-5" key={index}>
                <div className="w-60 md:w-40 h-100 border border-gray-200 rounded-lg shadow bg-gray-800 border-gray-700 z-0 transform transition-transform hover:scale-105 hover:shadow-md">
                  ...
                  <div className="p-2 cursor-pointer h-[160px] flex flex-col justify-between">
                    ...
                    >
                      <h5 className="mb-2 text-sm text-center font-medium tracking-tight text-gray-900 text-white">
                        {item?.nama}
                      </h5>
                    </a>
                    <div>
